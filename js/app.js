  ...
  handleImport(e) {
    ...
    reader.onload = (event) => {
      try {
        ...
        showToast(`瀵煎叆鎴愬姛锛佸叡 ${count} 鏉¤褰?馃摜`);
        this.render();
      } catch (err) {
        showToast(err.message || '瀵煎叆澶辫触', 'error');
      }
    };
    reader.onerror = () => {
      showToast('鏂囦欢璇诲彇澶辫触', 'error');
    };
    reader.readAsText(file);
    // 閲嶇疆 input 浠ユ敮鎸侀噸澶嶅鍏ュ悓鏂囦欢
